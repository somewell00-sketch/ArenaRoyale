...
// Non-invasive UX: also toast critical damage so it doesn't get missed.
try{
  const dmgMsgs = [];
  for(const e of (events || [])){
    if(!e) continue;
    if(e.type === "ATTACK" && e.target === "player"){
      const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
      const weap = w ? ` with ${w}` : "";
      dmgMsgs.push(`${(world?.entities?.npcs?.[e.who]?.name || e.who)} attacked you${weap} for ${Number(e.dmg || 0)} damage.`);
    } else if(e.type === "ATTACK_BLOCKED" && e.target === "player"){
      const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
      const weap = w ? ` (${w})` : "";
      dmgMsgs.push(`Your shield blocked an attack${weap}.`);
    } else if(e.type === "SHIELD_BREAK" && e.target === "player"){
      const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
      const weap = w ? ` (${w})` : "";
      dmgMsgs.push(`Your shield was broken${weap}.`);
    } else if(e.type === "THREAT" && e.target === "player"){
      if(e.kind === "creature") dmgMsgs.push(`${e.creature || "A creature"} attacked you for ${Number(e.dmg || 0)} damage.`);
      else dmgMsgs.push(`A hazard dealt ${Number(e.dmg || 0)} damage.`);
    } else if(e.type === "CREATURE_ATTACK" && e.who === "player"){
      dmgMsgs.push(`${e.creature} attacked you. You lost ${Number(e.dmg || 0)} HP.`);
    } else if(e.type === "MINE_HIT" && e.who === "player"){
      dmgMsgs.push(`A mine hit you for ${Number(e.dmg || 0)} damage.`);
    } else if(e.type === "SPLASH_DAMAGE" && e.who === "player"){
      const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
      const weap = w ? ` (${w})` : "";
      dmgMsgs.push(`You were caught in an explosion${weap} and took ${Number(e.dmg || 0)} damage.`);
    } else if(e.type === "HOSTILE_EVENT" && e.who === "player"){
      dmgMsgs.push(`Arena hazard: ${Number(e.dmg || 0)} damage.`);
    } else if(e.type === "LASER" && e.who === "player"){
      dmgMsgs.push(`Laser: ${Number(e.dmg || 0)} damage.`);
    }
  }
  for(const msg of dmgMsgs) pushToast(msg, { kind:"danger" });
} catch(_e){}
  ...
  function formatEvents(events){
    ...
    for(const e of (events || [])){
      switch(e.type){
        case "ATTACK": {
  // Player-initiated attack (commit action)
  if(e.ok){
    if(e.who && e.who !== "player"){
      // NPC attack resolved during commit-action phase or endDay
      if(e.target === "player"){
        const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
        const weap = w ? ` with ${w}` : "";
        out.push(`${npcName(e.who)} attacked you${weap} for ${Number(e.dmg || 0)} damage.`);
      }
      break;
    }
        case "ATTACK_BLOCKED": {
  // NPC attack blocked by your shield
  if(e.target === "player"){
    const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
    const weap = w ? ` (${w})` : "";
    out.push(`Your shield blocked an attack from ${npcName(e.who)}${weap}.`);
  }
  break;
}
case "SHIELD_BREAK": {
  if(e.target === "player"){
    const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
    const weap = w ? ` (${w})` : "";
    out.push(`${npcName(e.by)} broke your shield${weap}.`);
  } else {
    out.push(`${npcName(e.target)}'s shield was broken.`);
  }
  break;
}
case "SPLASH_DAMAGE": {
  if(e.who === "player"){
    const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
    const weap = w ? ` (${w})` : "";
    out.push(`You were caught in an explosion from ${npcName(e.by)}${weap} and took ${Number(e.dmg || 0)} damage.`);
  }
  break;
}
case "MINE_HIT": {
  if(e.who === "player") out.push(`A mine hit you for ${Number(e.dmg || 0)} damage.`);
  break;
}
case "THREAT": {
  if(e.target === "player"){
    const dmg = Number(e.dmg || 0);
    if(e.kind === "creature") out.push(`${e.creature || "A creature"} attacked you for ${dmg} damage.`);
    else out.push(`You were hurt by a hazard for ${dmg} damage.`);
  }
  break;
}
case "HOSTILE_EVENT": {
  if(e.who === "player") out.push(`You took ${Number(e.dmg || 0)} damage from the arena.`);
  break;
}
case "LASER": {
  if(e.who === "player") out.push(`A laser hit you for ${Number(e.dmg || 0)} damage.`);
  break;
}
...
// Also surface a quick toast summary (keeps the modal intact).
if(cannonCount > 0){
  ...
  pushToast([`Fallen tributes:`, `${list.join(", ")}${suffix}`], { kind:"event" });
    ...
    // Player damage during the day rollover (poison ticks, mines, daily threats, NPC attacks, etc.).
const rolloverDmg = (events || []).filter(e => {
  if(!e) return false;
  // endDay uses { who:"player" } for some, and { target:"player" } for THREAT.
  if(e.who === "player") return ["POISON_TICK","MINE_HIT","CREATURE_ATTACK","SPLASH_DAMAGE","HOSTILE_EVENT","LASER","SELF_DAMAGE","DAMAGE_RECEIVED"].includes(e.type);
  if(e.type === "ATTACK" && e.target === "player") return true;
  if(e.type === "ATTACK_BLOCKED" && e.target === "player") return true;
  if(e.type === "SHIELD_BREAK" && e.target === "player") return true;
  if(e.type === "THREAT" && e.target === "player") return true;
  return false;
});
    });
...
for(const e of rolloverDmg){
  if(e.type === "POISON_TICK") dmgLines.push(`Poison dealt ${e.dmg} damage.`);
    ...
    if(e.kind === "creature") dmgLines.push(`${e.creature || "A creature"} attacked you for ${e.dmg} damage.`);
    else dmgLines.push(`A hazard dealt ${e.dmg} damage.`);
  }
  else if(e.type === "ATTACK"){
    const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
    const weap = w ? ` (${w})` : "";
    dmgLines.push(`${npcName(e.who)} attacked you${weap} for ${e.dmg} damage.`);
  }
  else if(e.type === "ATTACK_BLOCKED"){
    const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
    const weap = w ? ` (${w})` : "";
    dmgLines.push(`Your shield blocked an attack from ${npcName(e.who)}${weap}.`);
  }
  else if(e.type === "SHIELD_BREAK"){
    const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
    const weap = w ? ` (${w})` : "";
    dmgLines.push(`${npcName(e.by)} broke your shield${weap}.`);
  }
  else if(e.type === "SPLASH_DAMAGE"){
    const w = e.weaponDefId ? (getItemDef(e.weaponDefId)?.name || e.weaponDefId) : null;
    const weap = w ? ` (${w})` : "";
    dmgLines.push(`Explosion from ${npcName(e.by)}${weap}: ${e.dmg} damage.`);
  }
  else if(e.type === "HOSTILE_EVENT") dmgLines.push(`Arena hazard: ${e.dmg} damage.`);
  else if(e.type === "LASER") dmgLines.push(`Laser: ${e.dmg} damage.`);
  else if(e.type === "SELF_DAMAGE") dmgLines.push(`Self-damage: ${e.dmg} damage.`);
  else if(e.type === "DAMAGE_RECEIVED") dmgLines.push(`Damage: ${e.dmg}.`);
}
